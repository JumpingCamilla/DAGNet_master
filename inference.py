import time
from models.dadepth import DADepth
import torch
...
class Inference:
    ...
    def init_model(self):
        ...
        with torch.no_grad():
            ...
            self.model = dadepth(
                ...
                opt.semantic_guidance, opt.depth_guidance, opt.feature_fusion
            )
    ...
    def inference(self):
        ...
        for i in range(max_iter):
            ...
            for image_path in glob.glob(self.image_dir + '/*.jpg'):
            # for image_path in glob.glob(self.image_dir+ '/*.png'):
                ...
                with torch.no_grad():
                    ...
                    flops, params = profile(self.model, inputs=(self.batch,), verbose=False)
                    flops, params = clever_format([flops, params], "%.3f")
                    print("\n  " + ("flops: {0}, params: {1}").format(flops, params))

                    n_params = sum(p.numel() for p in self.model.parameters())
                    n_params = n_params / 1e6
                    print(f'Total parameters: {n_params:.2f} M')
                    flop_counter = FlopCountAnalysis(self.model, self.batch)
                    gflops = flop_counter.total() / 1e9  # 将 FLOPs 转换为 GFLOPs
    ...
    def save_pred_to_disk(self, segs_pred, depth_pred):
        ...
        conc_img = np.concatenate((color_img, depth_img), axis=0)
        ...
        print('Saving...')
        cv2.imwrite(str(self.output_path + img_name + self.output_format), depth_img)
        # cv2.imwrite(str(self.output_path + '/seg_img/' + img_name + self.output_format), seg_img)
