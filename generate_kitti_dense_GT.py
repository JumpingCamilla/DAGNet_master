...
import torch
COLOR_SCHEME_PLASMA =torch .tensor((
...
for path in paths:
    ...
    max_value = gt[gt < np.inf].max().item()
    gt = gt/max_value
    gt[gt==np.inf] = np.nan

    ## if COLORMAP is PLASMA
    gt = torch.tensor(np.expand_dims(gt, axis=0))
    gt = depth_to_plasma(gt).squeeze()
    gt = gt.numpy().transpose(1,2,0)
    gt = (gt*255).astype('uint8')
    _,img_name = os.path.split(path)
    save_path = os.path.join('C:/Users/Administrator/Desktop',img_name)
    imsave(save_path,gt)

    ## if COLORMAP is 'rainbow','magma'or 'bone
    #gt = COLORMAPS['rainbow'](gt).astype(np.float32)
    # gt = gt.transpose(2, 0, 1)[:3]
    # gt = (gt*255).astype('uint8')
    # save_path = 'C:/Users/Administrator/Desktop/d_00066.png'
    # imsave(save_path, np.transpose(gt, (1,2,0)))
