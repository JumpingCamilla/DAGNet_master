...
class StateManager(object):
    def __init__(self, experiment_class, model_name, device, split_pos, num_layers,
                 ...
                 learning_rate, weight_decay, scheduler_step_size, semantic_guidance, depth_guidance, feature_fusion):
        ...
        self._init_model(
            ...
            num_layers_pose, semantic_guidance, depth_guidance, feature_fusion
        )
    ...
    def _init_model(self, split_pos, num_layers, grad_scale_depth, grad_scale_seg, weights_init, resolutions_depth,
        ...
        model = SGDepth(split_pos, num_layers, grad_scale_depth, grad_scale_seg, weights_init,
                        resolutions_depth, num_layers_pose, semantic_guidance, depth_guidance, feature_fusion
                        )
    ...
    def _init_optimizer(self, learning_rate, weight_decay, scheduler_step_size):
        with self.model_manager.get_train() as model:
            #self.optimizer = optim.Adam(model.parameters(), learning_rate)#self.optimizer = optim.Adam(model.parameters(), learning_rate, weight_decay=weight_decay)
            self.optimizer = Adan(model.parameters(), 1e-4, weight_decay=0.02, betas=[0.98,0.92,0.99],
                 eps=1e-8,
                 max_grad_norm=0.0, no_prox=False)

        self.lr_scheduler = optim.lr_scheduler.StepLR(
            self.optimizer, scheduler_step_size, weight_decay #self.optimizer, scheduler_step_size, learning_rate
        )
