...
## 22/06/22 Ying
def Make3D_validation(img_height, img_width, batch_size, num_workers):
    """A loader that loads images and depth ground truth for
    depth validation from the Make3D validation set.
    """
    ...
    transforms = [
        ...
        tf.ToTensor(),
        tf.NormalizeZeroMean(),
        tf.AddKeyValue('domain', 'Make3D_val_depth'),
        tf.AddKeyValue('validation_mask', 'validation_mask_Make3D'),
        tf.AddKeyValue('validation_clamp', 'validation_clamp_Make3D'),
        tf.AddKeyValue('purposes', ('depth', )),
    ]
